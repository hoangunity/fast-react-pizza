        ...
        {cart.map((item) => {
          return (
            <OrderItem
              ...
              key={item.pizzaId}
              ingredients={
                // If the data has not yet resolved, returned an empty
                // array explicitly to avoid error
                fetcher?.data?.find((el) => el.id === item.pizzaId)
                  ?.ingredients ?? []
              }
              // fetcher.state can hold 3 values "idle, loading, rejected"
...
export async function loader({ params }) {
  // orderId comes from the params string
