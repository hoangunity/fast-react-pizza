    ...
    <div className="px-4 py-6">
      ...
      <Form method="POST">
        ...
        <div className="mb-12 flex items-center gap-5">
          <input
            className="h-6 w-6 accent-yellow-400 focus:bg-yellow-300 focus:outline-none focus:ring focus:ring-yellow-400 focus:ring-offset-2"
            type="checkbox"
            name="priority"
            id="priority"
            value={withPriority}
        ...
        <div>
          {/* We use JSON.stringify() because the value can only be a string */}
...
export async function action({ request }) {
  ...
  const order = {
    ...data,
    cart: JSON.parse(data.cart), // cart is at JSON format (from the above input) => so we need to parse it back to JS object
